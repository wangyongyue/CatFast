...
class Panel003Cell: CollectionTemplate {
    var holder = Panel003CellHolder()
    override init(frame: CGRect) {
        ...
        navigationLabel.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(Screen.top() + 10)
        }
...
class Panel003CellModel: Cat {
    
    var name:String?
    var image:UIImage?

}
class Panel003CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is Panel003CellModel{
            let a = amodel as! Panel003CellModel
