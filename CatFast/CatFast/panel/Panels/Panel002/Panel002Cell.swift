...
class Panel002Cell: CollectionTemplate {
    var holder = Panel002CellHolder()
...
class Panel002CellModel: Cat {
    
    var name:String?
    var image:UIImage?

}
class Panel002CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is Panel002CellModel{
            let a = amodel as! Panel002CellModel
