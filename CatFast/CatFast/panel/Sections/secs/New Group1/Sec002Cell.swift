...
class Sec002Cell: CollectionTemplate {
    var holder = Sec002CellHolder()
...
class Sec002CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class Sec002CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is Sec002CellModel{
            let a = amodel as! Sec002CellModel
