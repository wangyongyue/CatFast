...
class Sec005Cell: CollectionTemplate {
    var holder = Sec005CellHolder()
...
class Sec005CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class Sec005CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is Sec005CellModel{
            let a = amodel as! Sec005CellModel
