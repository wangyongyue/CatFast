...
class Sec004Cell: CollectionTemplate {
    var holder = Sec004CellHolder()
...
class Sec004CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class Sec004CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is Sec004CellModel{
            let a = amodel as! Sec004CellModel
