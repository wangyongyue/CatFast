...
class ComTab001Cell: CollectionTemplate {
    var holder = ComTab001CellHolder()
...
class ComTab001CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComTab001CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComTab001CellModel{
            let a = amodel as! ComTab001CellModel
