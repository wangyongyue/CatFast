...
class ComTab003Cell: CollectionTemplate {
    var holder = ComTab003CellHolder()
...
class ComTab003CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComTab003CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComTab003CellModel{
            let a = amodel as! ComTab003CellModel
