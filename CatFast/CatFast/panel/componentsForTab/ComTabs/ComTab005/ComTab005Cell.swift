...
class ComTab005Cell: CollectionTemplate {
    var holder = ComTab005CellHolder()
...
class ComTab005CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComTab005CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComTab005CellModel{
            let a = amodel as! ComTab005CellModel
