...
class ComTab004Cell: CollectionTemplate {
    var holder = ComTab004CellHolder()
...
class ComTab004CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComTab004CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComTab004CellModel{
            let a = amodel as! ComTab004CellModel
