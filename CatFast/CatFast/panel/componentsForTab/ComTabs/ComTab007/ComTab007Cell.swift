...
class ComTab007Cell: CollectionTemplate {
    var holder = ComTab007CellHolder()
...
class ComTab007CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComTab007CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComTab007CellModel{
            let a = amodel as! ComTab007CellModel
