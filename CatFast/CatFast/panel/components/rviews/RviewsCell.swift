...
class RviewsCell: CollectionTemplate {
    var holder = RviewsCellHolder()
...
class RviewsCellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is RviewsCellModel{
            let a = amodel as! RviewsCellModel
