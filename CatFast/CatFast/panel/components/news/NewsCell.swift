...
class NewsCell: CollectionTemplate {
    var holder = NewsCellHolder()
...
class NewsCellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is NewsCellModel{
            let a = amodel as! NewsCellModel
