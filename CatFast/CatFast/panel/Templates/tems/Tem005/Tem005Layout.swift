...
class Tem005Layout: CCustomLayout {
    // 内容区域总大小，不是可见区域
    override var collectionViewContentSize: CGSize {
        let width = collectionView!.bounds.size.width - collectionView!.contentInset.left
            - collectionView!.contentInset.right
        
        let height = CGFloat((collectionView!.numberOfItems(inSection: 0))/2 + 1) * (width / 2)
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath)
        -> UICollectionViewLayoutAttributes? {
            ...
            let heigthCellSide = collectionViewContentSize.width/2
            
            let widthFirstCellSide = collectionViewContentSize.width
            ...
            if index == 0{
                ...
                //当前行数
                let line:Int =  (indexPath.item - 1) / 2 + 1
                let x:CGFloat = offset
