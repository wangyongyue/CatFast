...
class ComBot003Cell: CollectionTemplate {
    var holder = ComBot003CellHolder()
...
class  ComBot003CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot003CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot003CellModel{
            let a = amodel as! ComBot003CellModel
