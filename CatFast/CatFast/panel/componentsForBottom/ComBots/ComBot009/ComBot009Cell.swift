...
class ComBot009Cell: CollectionTemplate {
    var holder = ComBot009CellHolder()
...
class  ComBot009CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot009CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot009CellModel{
            let a = amodel as! ComBot009CellModel
