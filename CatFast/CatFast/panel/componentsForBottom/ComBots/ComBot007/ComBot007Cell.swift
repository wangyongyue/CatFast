...
class ComBot007Cell: CollectionTemplate {
    var holder = ComBot007CellHolder()
...
class  ComBot007CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot007CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot007CellModel{
            let a = amodel as! ComBot007CellModel
