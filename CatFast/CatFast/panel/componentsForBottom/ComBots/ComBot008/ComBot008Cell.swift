...
class ComBot008Cell: CollectionTemplate {
    var holder = ComBot008CellHolder()
...
class  ComBot008CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot008CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot008CellModel{
            let a = amodel as! ComBot008CellModel
