...
class ComBot001Cell: CollectionTemplate {
    var holder = ComBot001CellHolder()
...
class  ComBot001CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot001CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot001CellModel{
            let a = amodel as! ComBot001CellModel
