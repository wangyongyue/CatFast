...
class ComBot002Cell: CollectionTemplate {
    var holder = ComBot002CellHolder()
...
class  ComBot002CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot002CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot002CellModel{
            let a = amodel as! ComBot002CellModel
