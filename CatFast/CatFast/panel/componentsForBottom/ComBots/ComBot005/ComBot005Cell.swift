...
class ComBot005Cell: CollectionTemplate {
    var holder = ComBot005CellHolder()
...
class  ComBot005CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class  ComBot005CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComBot005CellModel{
            let a = amodel as! ComBot005CellModel
