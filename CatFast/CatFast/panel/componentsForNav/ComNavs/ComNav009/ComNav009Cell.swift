...
class ComNav009Cell: CollectionTemplate {
    var holder = ComNav009CellHolder()
...
class ComNav009CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComNav009CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComNav009CellModel{
            let a = amodel as! ComNav009CellModel
