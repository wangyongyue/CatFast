...
class ComNav004Cell: CollectionTemplate {
    var holder = ComNav004CellHolder()
...
class ComNav004CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComNav004CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComNav004CellModel{
            let a = amodel as! ComNav004CellModel
