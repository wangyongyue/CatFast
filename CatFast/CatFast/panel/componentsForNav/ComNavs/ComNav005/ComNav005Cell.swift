...
class ComNav005Cell: CollectionTemplate {
    var holder = ComNav005CellHolder()
...
class ComNav005CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComNav005CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComNav005CellModel{
            let a = amodel as! ComNav005CellModel
