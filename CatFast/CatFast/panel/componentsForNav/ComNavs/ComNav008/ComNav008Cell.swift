...
class ComNav008Cell: CollectionTemplate {
    var holder = ComNav008CellHolder()
...
class ComNav008CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComNav008CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComNav008CellModel{
            let a = amodel as! ComNav008CellModel
