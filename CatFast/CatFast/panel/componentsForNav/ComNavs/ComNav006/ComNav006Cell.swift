...
class ComNav006Cell: CollectionTemplate {
    var holder = ComNav006CellHolder()
...
class ComNav006CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComNav006CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComNav006CellModel{
            let a = amodel as! ComNav006CellModel
