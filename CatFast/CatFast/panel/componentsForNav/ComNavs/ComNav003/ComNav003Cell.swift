...
class ComNav003Cell: CollectionTemplate {
    var holder = ComNav003CellHolder()
...
class ComNav003CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class ComNav003CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is ComNav003CellModel{
            let a = amodel as! ComNav003CellModel
