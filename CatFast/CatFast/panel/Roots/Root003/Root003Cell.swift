...
class Root003Cell: CollectionTemplate {
    var holder = Root003CellHolder()
...
class Root003CellModel: Cat {
    
    var name:String?
    var image:UIImage?
    
}
class Root003CellHolder: Holder {
    var textOb = Observe()
    var imageOb = Observe()
    
    override func setModel(_ amodel: Cat) {
        super.setModel(amodel)
        if amodel is Root003CellModel{
            let a = amodel as! Root003CellModel
